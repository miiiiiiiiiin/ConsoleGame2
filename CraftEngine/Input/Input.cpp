...
namespace Craft
{
	...
	Input::Input()
	{
		...
		{
			OutputDebugStringA("Input: invalid handle!\n");
			return;
		}
		...
		if (GetConsoleMode(inputHandle, &originalConsoleMode))
		{
			...
			OutputDebugStringA(shouldRestoreConsoleMode ? "Mouse mode set OK\n" : "Mouse mode set FAILED\n");
		}
		//OutputDebugStringA("GetConsoleMode FAILED\n");
	}
			...
			&& pendingEventCount > 0)
		{
			...
			for (DWORD ix = 0; ix < readEventCount; ++ix)
			{
				...
				case KEY_EVENT:
				{
					...
					OutputDebugStringA("Input: KEY_EVENT OK!\n");
				...
				case MOUSE_EVENT:
				{
					...
					OutputDebugStringA("Input: MOUSE_EVENT OK!\n");
