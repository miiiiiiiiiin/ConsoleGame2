...
bool QuadTreeNode::Insert(std::shared_ptr<Craft::Actor>& actor)
{
	...
	if (!isDivided)
	{
		...
		{
			if (topleft->Insert(actor)) continue;
			if (topright->Insert(actor)) continue;
			if (bottomleft->Insert(actor)) continue;
			if (bottomright->Insert(actor)) continue;
		}
...
{
	// 겹치지도 않으면 걍 안봄
	if (!bounds.Intersects(targeBbounds)) return nullptr;
